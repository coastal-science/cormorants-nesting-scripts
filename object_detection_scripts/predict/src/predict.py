...
e.g.
python3 predict.py \
--tiles ../input/SNB_14_10062021 \
--exported_model ../input/models/snb1_centernet_resetnet101_512_v3/ \
...
def inference_as_raw_output(path2images, detection_model, box_th=0.25, nms_th=0.5):
    """
...
def predict_for_all_tiles(exported_model_dir, tile_dir, box_thresh):
    # Load the model
    detection_model, checkpoint = load_model(exported_model=exported_model_dir)

    # Use the detection model to detect objects in the tiles
    detections = inference_as_raw_output(tile_dir,
                                         detection_model,
                                         box_th=box_thresh)

    detection_df = pd.DataFrame(detections).transpose()

    detect_per_row = detection_df.explode(column=['detection_scores',
                                                  'detection_classes',
                                                  'detection_boxes'])
    detect_per_row['detection_boxes'] = detect_per_row['detection_boxes'].apply(
        lambda x: x.tolist() if isinstance(x, np.ndarray) else x)

    detect_per_row.index.name = 'image'

    return detect_per_row


def draw_detections(detections, threshold, save_dir=None):
...
if __name__ == '__main__':
    ...
    df = predict_for_all_tiles(exported_model_dir=Path(args.exported_model),
    ...
    df.to_csv(out_dir.joinpath('detections.csv'), index_label='image')

    if args.draw:
        draw_detections(df, threshold=args.box_thresh, save_dir=out_dir)